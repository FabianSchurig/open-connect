...
- **Verification:** Test (integration) — issue manifests specifying a `DOCKER_CONTAINER` primitive with `mode: CACHE_ONLY` and `mode: REPLACE_RUNNING_CONTAINER`; assert image digest match, coordinated compose cutover or container swap, and rollback on failure.
- **Priority:** Should.
